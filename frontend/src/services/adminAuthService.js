...
// Authenticate Admin User (API first, local fallback)
export const authenticateAdmin = async (emailOrUsername, password) => {
  const apiRes = await apiFetch(API_ENDPOINTS.AUTH_LOGIN, {
    method: 'POST',
    body: JSON.stringify({ emailOrUsername, password })
  });

  if (apiRes.success && apiRes.user) {
    sessionStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(apiRes.user));
...
export const createAdminAccount = async ({
  ...
  apiFetch(API_ENDPOINTS.AUTH_USERS, {
    method: 'POST',
    body: JSON.stringify({ name, username, email, password, role, customPermissions, avatar })
  });
...
export const updateAdminAccount = async (id, updateFields) => {
  ...
  apiFetch(`${API_ENDPOINTS.AUTH_USERS}/${id}`, {
    method: 'PUT',
...
export const deleteAdminAccount = (id) => {
  ...
  apiFetch(`${API_ENDPOINTS.AUTH_USERS}/${id}`, { method: 'DELETE' });
